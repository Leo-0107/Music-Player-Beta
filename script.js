...
(() => {
  ...
  /* 波形描画処理（上から見下ろす・右側アングル・左から右に流れる視点に変更） */
  function drawWaveform() {
    ...
    if (state.waveMode === "3d") {
      ...
      if (waveHistory3d.length > 28) waveHistory3d.pop();

      wave3dAngle += 0.015;

      const numHist = waveHistory3d.length;
      const numPoints = 64;

      // 上から見下ろす・右側からのアングルで奥から手前へソートして描画
      for (let zIdx = numHist - 1; zIdx >= 0; zIdx--) {
        const frame = waveHistory3d[zIdx];
        
        // 曲が左から右へ流れる処理 (zIdx=0が最新で左側、時間が経つ(zIdx大)につれて右側へ流れる)
        const progress = zIdx / numHist; // 0.0(最新:左) -> 1.0(過去:右)
        const posX = w * 0.08 + progress * (w * 0.72);
        
        // 上から見下ろす俯瞰視点 ＋ 右側アングルのパースペクティブ位置計算
        const baseY = h * 0.3 + (progress * h * 0.25);
        const alpha = Math.pow(1 - progress * 0.7, 1.5);

        ctx.beginPath();

        for (let i = 0; i < numPoints; i++) {
          const val = frame[i] * 0.38; // 音量の高さ（見下ろし視点のため上方向へ立体的に突出）
          
          // 周波数成分(i)を斜め奥から手前方向へ展開
          const px = posX + (i / numPoints) * (w * 0.18);
          const py = baseY - val - (i * 0.6); // 俯瞰のアングル感（奥ほど画面上方）

          if (i === 0) ctx.moveTo(px, py);
          else {
            const prevVal = frame[i - 1] * 0.38;
            const prevPx = posX + ((i - 1) / numPoints) * (w * 0.18);
            const prevPy = baseY - prevVal - ((i - 1) * 0.6);
        ...
        const hue = (190 + progress * 100 + wave3dAngle * 20) % 360;
        ctx.strokeStyle = `hsla(${hue}, 85%, 60%, ${alpha})`;
        ctx.lineWidth = Math.max(1, 2.2 * (1 - progress * 0.3));
        ctx.shadowBlur = zIdx === 0 ? 8 : 0;
        ctx.shadowColor = `hsla(${hue}, 85%, 60%, 0.8)`;
        ctx.stroke();

        // 3Dメッシュ格子線（フレーム間を補間して見下ろし立体感を高める）
        if (zIdx < numHist - 1 && zIdx % 2 === 0) {
          const nextFrame = waveHistory3d[zIdx + 1];
          const nextProgress = (zIdx + 1) / numHist;
          const nextPosX = w * 0.08 + nextProgress * (w * 0.72);
          const nextBaseY = h * 0.3 + (nextProgress * h * 0.25);
          ...
          for (let i = 0; i < numPoints; i += 5) {
            const px1 = posX + (i / numPoints) * (w * 0.18);
            const py1 = baseY - (frame[i] * 0.38) - (i * 0.6);

            const px2 = nextPosX + (i / numPoints) * (w * 0.18);
            const py2 = nextBaseY - (nextFrame[i] * 0.38) - (i * 0.6);
